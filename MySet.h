...
private:
	...
	Node<T>* copy(Node<T> *root, Node<T> *parent);
...
template <class T>
Node<T>* MySet<T>::copy(Node<T> *root, Node<T> *parent) {
	if (root == NULL) return NULL;
	Node<T> *el = new Node<T>(root->elem);
	el->parent = parent;
	el->right = copy(root->right, el);
	el->left = copy(root->left, el);
	return el;
